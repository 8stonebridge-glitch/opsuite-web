...
export interface DonorMatchResult {
  ...
  confidence_boost: number; // 0-0.3 based on match quality
 ...
 */
function extractFeatureKeywords(feature: CandidateFeature): string[] {
...
function computeKeywordOverlap(
  ...
  if (donorKeywords.length === 0 || featureKeywords.length === 0) return 0;
 ...
 */
function featureTypeOverlap(
  featureType: string,
  donorFeatureArea: string,
): number {
  const ft = featureType.toLowerCase();
  const da = donorFeatureArea.toLowerCase();

  if (ft === da) return 1.0;
  if (da.includes(ft) || ft.includes(da)) return 0.8;
  ...
  for (const area of da.split(",").map(a => a.trim())) {
    if (related.includes(area)) return 0.7;
  }
 ...
 */
export function matchFeatureToDonors(
  feature: CandidateFeature,
  kb: DonorKnowledgeBase,
): DonorMatchResult {
  ...
  let routing: DonorMatchResult["routing_recommendation"];
  ...
  const donorMappings: DonorMapping[] = matchedObs.map(obs => ({
    donor_name: obs.donor_name,
    donor_feature: obs.observation_id,
    relevance: matchScore >= 0.5 ? "direct" as const : "analogous" as const,
 ...
 */
export function matchAllFeatures(
 ...
 */
export function enrichFeaturesWithDonorMatches(
  features: CandidateFeature[],
  kb: DonorKnowledgeBase,
): CandidateFeature[] {
  return features.map(feature => {
    ...
    return {
      ...
      confidence: {
        ...(feature.confidence ?? {}),
        overall: Math.min(1.0,
          (feature.confidence?.overall ?? 0.5) + match.confidence_boost
        ),
 ...
 */
export function parseStudyPacket(
  markdown: string,
  donorName: string,
): DonorKnowledgeBase {
  const observations: DonorObservation[] = [];
  const logic_candidates: DonorLogicCandidate[] = [];
  const validators: DonorValidatorReq[] = [];
  ...
  while ((obsMatch = obsRegex.exec(markdown)) !== null) {
    ...
    observations.push({
      ...
      confidence: conf as DonorObservation["confidence"],
  ...
  while ((logicMatch = logicRegex.exec(markdown)) !== null) {
    ...
    logic_candidates.push({
      ...
      confidence: conf as DonorLogicCandidate["confidence"],
  ...
  while ((valMatch = valRegex.exec(markdown)) !== null) {
    ...
    validators.push({
      ...
      pass_condition: pass,
      blocking_level: blocking as DonorValidatorReq["blocking_level"],
 ...
 */
export function mergeKnowledgeBases(...bases: DonorKnowledgeBase[]): DonorKnowledgeBase {
  return {
