...
import type { AppSpec, FeatureSpec, FrontendSurface, FailureState } from "../types/app-spec";
...
export class AppDecomposer {
  ...
  decompose(input: DecomposeAppInput): DecomposeAppResult {
    ...
    const features: FeatureSpec[] = candidate_features.map((cf) => {
      ...
      const enrichedFrontendSurfaces = enrichFrontendSurfaces(
        cf.frontend_surfaces ?? [],
        cf,
        research_summary,
      );
      const enrichedFailureStates = enrichFailureStates(
        cf.failure_states ?? [],
        cf,
        research_summary,
      );
      const enrichedAcceptanceCriteria = enrichAcceptanceCriteria(
        cf.acceptance_criteria ?? [],
        research_summary,
      );
      ...
      if (research_summary) {
        if (research_summary.comparable_products.length > 0) evidenceSources.push("perplexity-market");
        if (research_summary.frontend_patterns.common_screens.length > 0) evidenceSources.push("perplexity-frontend");
        if (research_summary.backend_architecture.recommended_patterns.length > 0) evidenceSources.push("perplexity-backend");
        if (research_summary.security.frontend_security.length > 0 || research_summary.security.backend_security.length > 0) evidenceSources.push("perplexity-security");
        if (research_summary.pitfalls.frontend_pitfalls.length > 0) evidenceSources.push("perplexity-pitfalls");
        if (evidenceSources.length === 0) evidenceSources.push("perplexity-research");
      }
  ...
  checkCoverage(
    features: FeatureSpec[],
    app: AppSpec,
    researchDepth: { domainsCovered: number; domainsTotal: number },
  ): CoverageReport {
 ...
 */
function enrichFrontendSurfaces(
  existing: FrontendSurface[],
  candidate: CandidateFeature,
  research?: StructuredResearchSummary,
): FrontendSurface[] {
  if (!research || existing.length > 0) return existing;
  ...
  const relevantScreens = research.frontend_patterns.common_screens.filter((s) => {
    ...
    return (
      featureNameLower.includes(screenLower) ||
  ...
  if (relevantScreens.length === 0) return existing;
 ...
 */
function enrichFailureStates(
  existing: FailureState[],
  candidate: CandidateFeature,
  research?: StructuredResearchSummary,
): FailureState[] {
  ...
  for (const uxState of research.frontend_patterns.empty_loading_error_states) {
    ...
    if (
      screenLower.includes(featureLower) ||
 ...
 */
function enrichAcceptanceCriteria(
  existing: FeatureSpec["acceptance_criteria"],
  research?: StructuredResearchSummary,
 ...
 */
function computeResearchDepth(summary: StructuredResearchSummary): ResearchDepthResult {
  const domains = [
    // Market (existing)
 ...
 */
function extractOperatorPitfalls(summary: StructuredResearchSummary): Pitfall[] {
  const all = [
