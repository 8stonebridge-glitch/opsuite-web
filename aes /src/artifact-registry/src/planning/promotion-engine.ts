...
import { ARTIFACT_META } from "../registry/artifact-meta";

// ─── ID Generation ─────────────────────────────────────────────────────────────

let evalCounter = 0;
function generateEvaluationId(): string {
  const prefix = ARTIFACT_META.promotion_evaluation.prefix;
...
function topologicalSort(features: FeatureSpec[]): {
  order: string[];
  hasCycle: boolean;
  missingDeps: string[];
} {
...
function evaluateFlowGate(features: FeatureSpec[]): PromotionGateResult {
  const issues: string[] = [];

  for (const f of features) {
    const hasMandatoryCriterion = f.acceptance_criteria.some(
      (c) => c.mandatory,
    );
    if (!hasMandatoryCriterion) {
      issues.push(
        `${f.feature_id} has no mandatory acceptance criteria`,
      );
    }
  }
...
function evaluateBuildabilityGate(
  features: FeatureSpec[],
): PromotionGateResult {
  const issues: string[] = [];
  ...
  for (const f of features) {
    ...
    if (!f.description || f.description.trim().length < 10) {
      issues.push(
        `${f.feature_id} has insufficient description (< 10 chars)`,
      );
    }
  }
...
function evaluateContradictionGate(
  features: FeatureSpec[],
): PromotionGateResult {
  const issues: string[] = [];
  ...
  for (const [entity, owners] of entityOwners) {
    ...
    if (uniqueOwners.length > 1) {
      issues.push(
        `Data entity "${entity}" claimed by multiple owners: ${uniqueOwners.join(", ")}`,
      );
    }
  }
...
function evaluateConfidenceThresholdGate(
  features: FeatureSpec[],
): PromotionGateResult {
  ...
  for (const f of features) {
    if (f.confidence_summary.overall < 0.6) {
      lowConfidence.push(
        `${f.feature_id} confidence ${f.confidence_summary.overall.toFixed(2)} < 0.60`,
      );
    }
  }
...
function evaluateMissingQuestionsGate(
  app: AppSpec,
  features: FeatureSpec[],
): PromotionGateResult {
  const issues: string[] = [];
  ...
  if (app.open_questions.length > 0) {
    issues.push(
      `App has ${app.open_questions.length} open questions: ${app.open_questions.join("; ")}`,
    );
  }
  ...
  for (const f of features) {
    if (f.missing_questions.length > 3) {
      issues.push(
        `${f.feature_id} has ${f.missing_questions.length} missing questions (max 3)`,
      );
    }
  }
 ...
 */
export function evaluateSpecHardVetoes(
  features: FeatureSpec[],
): SpecHardVeto[] {
  const vetoes: SpecHardVeto[] = [];
  ...
  for (const f of features) {
    ...
    for (const criterion of f.acceptance_criteria) {
      if (criterion.mandatory) {
        const hasLinkedTest = f.test_cases.some(
          (t) => t.linked_criterion_id === criterion.id,
        );
        if (!hasLinkedTest) {
          vetoes.push({
...
export function evaluatePromotion(
  ...
  if (verificationReport?.recommendation === "REJECT") {
    reasons.push(
      `Verification rejected: ${verificationReport.notes.join("; ")}`,
    );
  }

  let decision: PromotionEvaluation["decision"];
  if (hardBlocked) {
    decision = "BLOCKED";
...
export function computeDependencyOrder(
  features: FeatureSpec[],
): { order: string[]; hasCycle: boolean; missingDeps: string[] } {
