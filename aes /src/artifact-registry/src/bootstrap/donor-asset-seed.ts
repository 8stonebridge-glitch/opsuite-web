...
export type CanonicalFeatureClass =
  | "backend_platform"
...
interface FeatureTypeSeed {
  id: CanonicalFeatureClass;
  node_id: string;
  feature_class: CanonicalFeatureClass;
  display_name: string;
  summary: string;
...
interface PatternSeed {
  ...
  preferred_combinations: string[];
  source_donors: string[];
  source_artifact_ids: string[];
  evidence_refs: string[];
  accepted_components: string[];
  source_layer: "DERIVED";
  promotion_stage: "CANONICAL";
  authority_tier: AuthorityTier;
  enforceable: false;
...
interface ScenarioPackSeed {
  id: string;
  node_id: string;
  scenario_name: string;
  feature_class: CanonicalFeatureClass;
  source_feature_class: string;
...
interface FeatureBindingSeed {
  edge_id: string;
  feature_id: string;
  feature_type_id: CanonicalFeatureClass;
}
...
const FEATURE_TYPES: FeatureTypeSeed[] = [
  {
    id: "workflow",
...
const PATTERNS: PatternSeed[] = [
  {
    ...
    summary:
      "Keeps state, metadata, collaboration, assignee, project, subscribers, and next actions attached to one work object.",
    confidence: "high",
  ...
  },
  {
    ...
    summary:
      "Keeps notifications reason-aware, triageable, and linked to the object and workflow that caused them.",
    confidence: "high",
  ...
  },
  {
    ...
    summary:
      "Decomposes failed-payment handling into visible metrics, retries, emails, automations, recovery methods, and decline causes inside one operator surface.",
    confidence: "high",
  ...
  },
  {
    ...
    pattern_type: "ui_model",
    summary:
      "Separates channels, DMs, activity, and personal self-space while preserving rich collaboration actions and quiet-state continuity.",
    confidence: "high",
  ...
  },
  {
    ...
    pattern_type: "core_model",
    summary:
      "Makes post-auth setup, active organization context, role visibility, and switching or invitation flows explicit.",
    confidence: "high",
  ...
  },
  {
    ...
    pattern_type: "ui_model",
    summary:
      "Separates money movement, billing modules, reports, and debugging tools inside one operator shell with visible environment mode.",
    confidence: "high",
...
const VALIDATOR_BUNDLES: ValidatorBundleSeed[] = [
  {
...
const BRIDGE_PRESETS: BridgePresetSeed[] = [
  {
...
const SCENARIO_PACKS: ScenarioPackSeed[] = [
  {
...
const FEATURE_BINDINGS: FeatureBindingSeed[] = [
  {
...
const PATTERN_BUNDLE_EDGES: EdgeSeed[] = PATTERNS.flatMap((pattern) =>
  VALIDATOR_BUNDLES.filter((bundle) =>
    bundle.derived_from_patterns.includes(pattern.id)
  ).map((bundle, index) => ({
...
const PATTERN_PRESET_EDGES: EdgeSeed[] = PATTERNS.flatMap((pattern) =>
  BRIDGE_PRESETS.filter((preset) =>
    preset.derived_from_patterns.includes(pattern.id)
  ).map((preset, index) => ({
...
const PATTERN_SCENARIO_EDGES: EdgeSeed[] = [
  {
...
async function seedNodes<TRow extends object>(
...
export async function seedCanonicalDonorAssets(
  executor: Neo4jQueryExecutor
): Promise<DonorAssetSeedSummary> {
