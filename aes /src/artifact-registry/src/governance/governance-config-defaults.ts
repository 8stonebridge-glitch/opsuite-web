...
export function createDefaultTrainable(): TrainableGovernance {
  return {
    confidence_weights: {
      graph_coverage: 0.35,
...
export function createDefaultFrozen(): FrozenGovernance {
  return {
    hard_veto_codes: [
 ...
 */
export function createBaselineGovernanceConfig(): GovernanceConfig {
  return {
    governance_config_id: generateArtifactId("governance_config"),
    version: 1,
...
export function validateGovernanceConfig(
  ...
  for (const [key, val] of Object.entries(w)) {
    if (val < 0) issues.push(`Confidence weight ${key} is negative: ${val}`);
    if (val > 1) issues.push(`Confidence weight ${key} exceeds 1.0: ${val}`);
  }
