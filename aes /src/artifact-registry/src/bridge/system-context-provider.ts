 ...
 */
export function buildPostgresContext(): DatabaseContext["postgres"] {
  return {
    tables: [
      {
 ...
 */
export function buildNeo4jContext(): DatabaseContext["neo4j"] {
  return {
    node_labels: [
      {
        label: "FeatureSpec",
 ...
 */
export function buildSystemContext(featureType?: string): SystemContext {
  ...
  const featureSpecific: Record<string, unknown> = {};
 ...
 */
export function serializeContextForBridge(context: SystemContext): Record<string, unknown> {
  return {
    existing_database: {
