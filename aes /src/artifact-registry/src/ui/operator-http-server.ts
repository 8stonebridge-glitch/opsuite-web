...
import {
  ...
  extendedPanelsHtml,
  extendedPanelScripts,
} from "./agent-pipeline-view";
import { officeRendererScript } from "./office-renderer";
import {
  runBuildProgramWorkflow,
  runConfiguredBuilderWorkflow,
  type BuildProgramInput,
  type BuilderLaunchConfig,
  type RunBuilderWorkflowOptions,
} from "../bootstrap";
...
import type { AppIntakeInput } from "../intake/app-intake";
import { AppIntakeService } from "../intake/app-intake";
import { AppResearchService } from "../research/app-research";
import {
  AppDecomposer,
  type CandidateFeature,
} from "../planning/app-decomposer";
import { SpecVerifier } from "../planning/spec-verifier";
import {
  evaluatePromotion,
} from "../planning/promotion-engine";
import {
  generateBuildProgram,
} from "../planning/build-program-generator";
...
type RecordDiffPayload = Omit<RecordBuilderArtifactsInput, "build_id">;
type RecordTestRunPayload = Omit<RecordTestRunInput, "build_id">;
...
async function readJsonBody<T>(request: IncomingMessage): Promise<T> {
  ...
  try {
    ...
    throw new OperatorHttpRequestError(
      400,
      "INVALID_JSON",
...
function dashboardHtml(_baseUrl: string, summary: {
  pending_escalations: number;
  blocked_builds: number;
  verified_restricted_write_backs: number;
  stale_bridges: number;
}): string {
...
export class OperatorHttpServer {
  ...
  readonly agentTracker = new AgentActivityTracker();

  constructor(
    private readonly runtime: AesPlatformRuntime,
    private readonly dependencies: OperatorHttpServerDependencies = {}
  ) {}

  async start(
    options: OperatorHttpServerOptions = {}
  ): Promise<StartedOperatorHttpServer> {
    if (this.server) {
  ...
  private async handleRequest(
    ...
    try {
      ...
      if (method === "POST" && path === "/api/builds/prepare") {
        const payload = await readJsonBody<PrepareBuildInput>(request);
        json(response, 201, await this.runtime.prepareBuild(payload));
        return;
      }

      if (method === "POST" && path === "/api/build-programs/run") {
        const payload = await readJsonBody<BuildProgramInput>(request);
        json(
          response,
          200,
          await runBuildProgramWorkflow(
            this.runtime,
            this.requireBuilderConfig(),
            payload
          )
        );
        return;
      ...
      if (method === "POST" && path === "/api/agent-status/update") {
        ...
        if (payload.agent_id) {
          this.agentTracker.update(payload.agent_id, {
            state: (payload.state as any) || undefined,
            current_task: payload.current_task,
            feature_id: payload.feature_id,
            last_result: (payload.last_result as any) || undefined,
          });
        }
        if (payload.phase) this.agentTracker.setPhase(payload.phase);
        if (payload.app_id) this.agentTracker.setApp(payload.app_id);
        if (payload.feature_id) this.agentTracker.setFeature(payload.feature_id);
        if (payload.total_features !== undefined) {
          this.agentTracker.setTotals(
            payload.total_features,
            payload.completed_features ?? 0,
            payload.failed_features ?? 0,
            payload.blocked_features ?? 0,
          );
        }
        if (!this.agentTracker.getStatus().started_at) this.agentTracker.start();
      ...
      if (method === "GET" && path === "/api/attention-queue") {
        ...
        json(
          response,
          200,
          await this.runtime.attentionQueue({
      ...
      if (method === "GET" && path === "/api/governance/pending") {
        json(
          response,
          200,
          await this.runtime.services.governance_gateway.pendingDecisionQueue()
        );
        return;
      }
      ...
      if (method === "POST" && runBuilderMatch) {
        const payload = normalizeRunBuilderWorkflowOptions(
          await readJsonBody<RunBuilderWorkflowOptions>(request)
        );
        const buildId = runBuilderMatch[1]!;
        const result = await runConfiguredBuilderWorkflow(
          this.runtime,
          buildId,
          this.requireBuilderConfig(),
          payload
      ...
      if (method === "POST" && recordDiffMatch) {
        ...
        const payload = await readJsonBody<RecordDiffPayload>(request);
        json(response, 200, {
          build_id: buildId,
      ...
      const recordTestRunMatch = path.match(
        /^\/api\/builds\/([^/]+)\/record-test-run$/
      );
      if (method === "POST" && recordTestRunMatch) {
        const buildId = recordTestRunMatch[1]!;
        const payload = await readJsonBody<RecordTestRunPayload>(request);
        json(response, 200, {
          build_id: buildId,
      ...
      if (method === "GET" && escalationMatch) {
        json(
          response,
          200,
          await this.runtime.services.governance_gateway.decisionContext(
            escalationMatch[1]!
          )
        );
        return;
      ...
      const escalationDecisionMatch = path.match(
        /^\/api\/governance\/escalations\/([^/]+)\/(approve|reject|defer)$/
      );
      if (method === "POST" && escalationDecisionMatch) {
        const payload = await readJsonBody<GovernanceDecisionPayload>(request);
        ...
        if (action === "approve") {
          json(
            response,
            200,
            await this.runtime.services.governance_gateway.approveEscalation({
        ...
        if (action === "reject") {
          json(
            response,
            200,
            await this.runtime.services.governance_gateway.rejectEscalation({
              escalation_record_id: escalationRecordId,
              decided_by: payload.decided_by,
              rationale: payload.rationale,
              artifact_refs: payload.artifact_refs,
        ...
        json(
          response,
          200,
          await this.runtime.services.governance_gateway.deferEscalation({
            escalation_record_id: escalationRecordId,
            decided_by: payload.decided_by,
            rationale: payload.rationale,
            artifact_refs: payload.artifact_refs,
        ...
        html(
          ...
          dashboardHtml("", {
            ...
            verified_restricted_write_backs:
              attentionQueue.verified_restricted_write_backs.length,
      ...
      if (method === "POST" && path === "/api/app/intake") {
        const payload = await readJsonBody<AppIntakeInput>(request);
        const intake = new AppIntakeService(this.runtime.registry);
      ...
      if (method === "POST" && appResearchMatch) {
        ...
        const researchService = new AppResearchService(
          this.runtime.registry,
          this.runtime.services.research_gateway,
        );
      ...
      if (method === "POST" && appDecomposeMatch) {
        ...
        const payload = await readJsonBody<{
          ...
          research_summary?: import("../types/research-types").StructuredResearchSummary;
        }>(request);
      ...
      if (method === "POST" && appChecklistMatch) {
        ...
        const { parseResearchSummary } = await import("../research/app-research");
        ...
        const totalSteps =
          checklist.account_setup.length +
        ...
        json(response, 200, {
          ...
          total_security_items:
            security.frontend_security.length +
      ...
      if (method === "POST" && appVerifyMatch) {
        ...
        for (const fid of appRecord.payload.feature_ids) {
          const fRecord = await this.runtime.registry.read<FeatureSpec>(
            "feature_spec",
            fid,
          );
      ...
      if (method === "POST" && appPromoteMatch) {
        ...
        // Store evaluation
        await this.runtime.registry.write(
      ...
      if (method === "POST" && appSeedMatch) {
        ...
        const evalRecord = await this.runtime.registry.read(
          "promotion_evaluation",
          appId,
        );
        ...
        json(response, 200, {
          ...
          evaluation_decision: (evalRecord.payload as { decision?: string }).decision,
          message:
            "Features ready for graph seeding. Use build-program to execute.",
        });
        return;
      }

      const appBuildProgramMatch = path.match(
        /^\/api\/app\/([^/]+)\/build-program$/,
      );
      if (method === "POST" && appBuildProgramMatch) {
        ...
        const buildProgram = generateBuildProgram({
          app: appRecord.payload,
          features,
      ...
      if (method === "GET" && appStatusMatch) {
        ...
        for (const fid of appRecord.payload.feature_ids) {
          try {
            const fRecord =
              await this.runtime.registry.read<FeatureSpec>(
                "feature_spec",
                fid,
              );
            features.push(fRecord.payload);
      ...
      if (method === "GET" && path.startsWith("/assets/")) {
        const assetPath = pathModule.resolve(
          __dirname, "..", "..", "assets",
          decodeURIComponent(path.slice(8))
        );
        // Security: prevent directory traversal
        const assetsRoot = pathModule.resolve(__dirname, "..", "..", "assets");
  ...
  private requireBuilderConfig(): BuilderLaunchConfig {
    if (!this.dependencies.builder) {
      throw new OperatorHttpRequestError(
        503,
        "BUILDER_NOT_CONFIGURED",
        "Operator server does not have a builder launch configuration."
      );
    }
...
function normalizeRunBuilderWorkflowOptions(
  payload: RunBuilderWorkflowOptions
): RunBuilderWorkflowOptions {
  if (
    ...
    throw new OperatorHttpRequestError(
      400,
      "INVALID_TIMEOUT",
      "timeout_ms must be a positive number when provided."
    );
  }
