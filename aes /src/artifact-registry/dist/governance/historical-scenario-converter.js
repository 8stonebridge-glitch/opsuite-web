...
exports.loadHistoricalScenarios = loadHistoricalScenarios;
exports.loadAppScenarios = loadAppScenarios;
// ─── Per-Feature Scenario Extraction ─────────────────────────────────────────
/**
 * Extract a replay scenario from a single feature's build artifacts.
 */
function featureBuildToScenario(feature, bridge, build, validatorRuns, diff, testRuns, escalations) {
    // Determine validator consensus
    let validatorOutcome = "PASS";
    ...
    const riskDomainTags = [];
    if (feature.auth_requirements.length > 0)
        riskDomainTags.push("auth");
    if (feature.destructive_actions.length > 0)
        riskDomainTags.push("destructive");
    if (feature.data_entities.some(d => d.operations.includes("DELETE")))
        riskDomainTags.push("data");
    if (feature.integrations.some(i => i.type === "payment"))
        riskDomainTags.push("payments");
    if (feature.integrations.some(i => i.type === "oauth"))
        riskDomainTags.push("oauth");
    if (feature.events.length > 0)
        riskDomainTags.push("events");
    if (feature.feature_type === "notification_system")
        riskDomainTags.push("notifications");
    if (feature.feature_type === "collaboration_system")
        riskDomainTags.push("realtime");
    // Map build status
    let buildStatus;
    if (build.status === "PASSED")
        buildStatus = "PASSED";
    else if (build.status === "BLOCKED")
        buildStatus = "BLOCKED";
    else
        buildStatus = "FAILED";
    return {
 ...
 */
function generateCrossFeatureScenarios(app, features, featureBuilds) {
    const scenarios = [];
    let idx = 0;
 ...
 */
async function loadHistoricalScenarios(registry) {
    const scenarios = [];
    // Load all builds
    const buildRecords = await registry.latestByType("build");
    // Group builds by feature
    const featureBuilds = new Map();
    for (const buildRecord of buildRecords) {
        const build = buildRecord.payload;
        // Only process terminal builds
        if (!["PASSED", "FAILED", "BLOCKED"].includes(build.status))
            continue;
        ...
        try {
            // Get bridge
            const bridgeRecord = await registry.read("bridge", build.bridge_id);
            ...
            for (const artifact of buildArtifacts) {
                switch (artifact.artifact_type) {
                    case "diff_artifact":
                        diff = artifact.payload;
                        break;
                    case "validator_run":
                        validatorRuns.push(artifact.payload);
                        break;
                    case "test_run":
                        testRuns.push(artifact.payload);
                        break;
                    case "escalation_record":
                        escalations.push(artifact.payload);
                        break;
                }
            }
        }
        ...
        let feature = null;
        try {
            const featureRecord = await registry.read("feature_spec", build.feature_id);
        ...
        // Convert to scenario
        const scenario = featureBuildToScenario(feature, bridge, build, validatorRuns, diff, testRuns, escalations);
    ...
    const appRecords = await registry.latestByType("app_spec");
    for (const appRecord of appRecords) {
        const app = appRecord.payload;
        const appFeatures = [];
        for (const featureId of app.feature_ids) {
            try {
                const featureRecord = await registry.read("feature_spec", featureId);
 ...
 */
async function loadAppScenarios(registry, appId) {
    // Load the app spec
    const appRecord = await registry.read("app_spec", appId);
    const app = appRecord.payload;
    const scenarios = [];
    const featureBuilds = new Map();
    const appFeatures = [];
    for (const featureId of app.feature_ids) {
        ...
        const buildRecords = featureArtifacts.filter(r => r.artifact_type === "build");
        for (const buildRecord of buildRecords) {
            const build = buildRecord.payload;
            if (!["PASSED", "FAILED", "BLOCKED"].includes(build.status))
                continue;
 ...
 */
function createMinimalFeatureSpec(build, bridge) {
    return {
...
//# sourceMappingURL=historical-scenario-converter.js.map