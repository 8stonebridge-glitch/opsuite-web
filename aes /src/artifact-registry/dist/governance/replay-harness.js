...
exports.SCORING_FORMULA_V1 = void 0;
...
// ─── Scoring Formula ────────────────────────────────────────────────────────
exports.SCORING_FORMULA_V1 = "composite = 0.25 * bugs_caught + 0.25 * (1 - false_alarms) + 0.25 * (1 - blocked_valid) + 0.25 * outcome_improvement";
function computeCompositeScore(scores) {
    return (0.25 * scores.bugs_caught_rate +
 ...
 */
function simulateRoute(score, thresholds) {
    if (score >= thresholds.direct_build_floor)
        return "DIRECT_BUILD";
    if (score >= thresholds.caution_build_floor)
        return "CAUTION_BUILD";
    if (score >= thresholds.research_required_floor)
 ...
 */
function simulateConfidence(scenario, weights) {
    ...
    const graph_coverage = Math.max(0.05, Math.min(1.0, inp.bridge_confidence - scopePenalty - depPenalty));
    ...
    const pattern_strength = Math.max(0.05, Math.min(1.0, 0.55 + critBoost - riskPenalty + (inp.bridge_confidence - 0.7) * 0.5));
    ...
    const rule_consistency = Math.max(0.05, Math.min(1.0, 0.85 - depPenalty * 1.5 - filePenalty));
    // evidence_level: direct from bridge confidence, modulated by scope
    const evidence_level = Math.max(0.05, Math.min(1.0, inp.bridge_confidence * (1 - scopePenalty * 0.5)));
    return (weights.graph_coverage * graph_coverage +
 ...
 */
function replayScenario(scenario, config) {
 ...
 */
function runReplay(config, scenarios, baselineConfig) {
    const startedAt = new Date().toISOString();
    const results = scenarios.map((s) => replayScenario(s, config));
    const completedAt = new Date().toISOString();
    // Aggregate scores
    const totalScenarios = results.length;
    const realBugs = scenarios.filter((s) => s.actual_outcome.build_status === "FAILED" || s.actual_outcome.had_scope_violations || s.actual_outcome.had_test_failures);
    const validWork = scenarios.filter((s) => s.actual_outcome.build_status === "PASSED" && !s.actual_outcome.had_scope_violations);
    ...
    let baselineComposite = null;
    let delta = null;
    if (baselineConfig) {
        const baselineResults = scenarios.map((s) => replayScenario(s, baselineConfig));
        const baselineRealBugs = scenarios.filter((s) => s.actual_outcome.build_status === "FAILED" || s.actual_outcome.had_scope_violations || s.actual_outcome.had_test_failures);
        const baselineValidWork = scenarios.filter((s) => s.actual_outcome.build_status === "PASSED" && !s.actual_outcome.had_scope_violations);
    ...
    const runId = (0, id_generator_1.generateArtifactId)("replay_run");
    const reportId = (0, id_generator_1.generateArtifactId)("replay_report");
    const run = {
        replay_run_id: runId,
    ...
    const report = {
        ...
        scoring_formula: exports.SCORING_FORMULA_V1,
