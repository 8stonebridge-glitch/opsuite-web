...
class VerificationRunner {
     ...
     */
    async runChecks(buildId, projectDir, checks, pass) {
        const verificationId = (0, id_generator_1.generateArtifactId)("verification");
        const startedAt = new Date().toISOString();
        const startTime = Date.now();
        // Separate built-in and custom checks
        const builtinChecks = checks.filter((c) => c.type === "builtin");
        const customChecks = checks.filter((c) => c.type === "single");
        const results = [];
        ...
        if (builtinChecks.length > 0) {
            ...
            for (const pr of pipelineResults) {
                ...
                if (!pr.passed) {
                    const classification = (0, failure_classifier_1.classifyFailure)(pr.check, pr.output);
        ...
        for (const custom of customChecks) {
            const singleResult = await this.runner.runSingleCheck(projectDir, custom.command, custom.checkName);
            const result = {
            ...
            if (!singleResult.passed) {
                const classification = (0, failure_classifier_1.classifyFailure)(singleResult.checkName, singleResult.output);
                result.normalized_error_summary = classification.normalized_error_summary;
                result.failure_class = classification.failure_class;
                result.failing_files = classification.failing_files;
                result.failing_symbols = classification.failing_symbols;
     ...
     */
    static toRetryContext(result, attemptNumber) {
        const failedChecks = result.checks.filter(c => !c.passed);
        const allFailingFiles = [];
        const allFailingSymbols = [];
        const allClasses = new Set();
        for (const check of failedChecks) {
            if (check.failing_files) {
                for (const f of check.failing_files) {
                    if (!allFailingFiles.includes(f))
                        allFailingFiles.push(f);
                }
            }
            if (check.failing_symbols) {
                for (const s of check.failing_symbols) {
                    if (!allFailingSymbols.includes(s))
                        allFailingSymbols.push(s);
                }
            }
            if (check.failure_class)
                allClasses.add(check.failure_class);
        }
        ...
        return {
            ...
            failure_classes: Array.from(allClasses),
