...
exports.UiSurfaceObserver = void 0;
// ─── Observer ────────────────────────────────────────────────────────────────
class UiSurfaceObserver {
     ...
     */
    observe(feature, appId, researchPatterns) {
     ...
     */
    extractScreensFromResearch(feature, research) {
        ...
        for (const screen of research.common_screens) {
            ...
            const matches = featureName.includes(screenName) ||
     ...
     */
    extractStatesFromResearch(screens, research) {
        const states = [];
        for (const screen of screens) {
            ...
            if (!hasResearchStates) {
                states.push({ state_type: "loading", screen_id: screen.screen_id, description: `Loading state for ${screen.name}` }, { state_type: "empty", screen_id: screen.screen_id, description: `No data state for ${screen.name}`, user_action: "Create first item" }, { state_type: "error", screen_id: screen.screen_id, description: `Error state for ${screen.name}`, user_action: "Retry or go back" });
            }
        }
    ...
    extractScreens(feature) {
        const screens = [];
        const frontend = feature.frontend_surfaces?.[0];
    ...
    extractJourneys(feature, screens) {
        const journeys = [];
        if (screens.length > 0) {
            // Happy path: navigate through all screens
            const happyPath = screens.map((screen, i) => ({
                step: i + 1,
    ...
    inferLayout(feature, screens) {
        const intent = (feature.description || "").toLowerCase();
        let type = "list";
        let navigation = "none";
        if (intent.includes("dashboard"))
            type = "dashboard";
        else if (intent.includes("wizard") || intent.includes("onboarding"))
            type = "wizard";
        else if (intent.includes("settings") || intent.includes("config"))
            type = "settings";
        else if (intent.includes("detail") || intent.includes("profile"))
            type = "detail";
        else if (intent.includes("form") || intent.includes("create"))
            type = "form";
...
exports.UiSurfaceObserver = UiSurfaceObserver;
//# sourceMappingURL=ui-surface-observer.js.map