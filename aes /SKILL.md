...
# Skill Boundary Notice

This file is a specialized AES skill, not the default repository root instruction file.

If you are doing normal project work in this repository, follow `/CLAUDE.md` first.

Use this `SKILL.md` only when Claude is explicitly acting as the AES improvement-loop or orchestrator skill.

Do not treat this file as the primary instruction source for routine implementation tasks such as building or upgrading the AES operator interface.

# Claude Role Notice

You are Claude Code.

When this skill is loaded, you are acting in the specialized AES orchestrator or improvement-loop role.

Keep `/CLAUDE.md` as the base repository instruction file.
Use this skill to add orchestrator-specific behavior on top of that base.

If this skill conflicts with normal repo implementation guidance, prefer `/CLAUDE.md` for routine project work and use this file only for explicit improvement-loop or orchestrator tasks.
